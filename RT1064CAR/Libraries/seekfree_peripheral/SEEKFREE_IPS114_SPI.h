...
#endif
#else
#define IPS114_SCL_PIN          B0    //定义SPI_SCK引脚
#define IPS114_SDA_PIN          B1    //定义SPI_MOSI引脚
#define IPS114_REST_PIN         C18    //液晶复位引脚定义 由于实际通讯未使用D15因此 这里复用为GPIO
#define IPS114_DC_PIN 	        B3	  //液晶命令位引脚定义
#define IPS114_CS_PIN           C19   //定义SPI_CS引脚
#define IPS114_BL_PIN           B2	  //液晶背光引脚定义  


#define	IPS114_SCL(x)	gpio_set(IPS114_SCL_PIN,x)
#define IPS114_SDA(x)	gpio_set(IPS114_SDA_PIN,x)
#define IPS114_REST(x)	gpio_set(IPS114_REST_PIN,x)
#define IPS114_DC(x)	gpio_set(IPS114_DC_PIN,x)
#define IPS114_CS(x)	gpio_set(IPS114_CS_PIN,x)
