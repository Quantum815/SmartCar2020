...
uint8 DebugRx_Buffer,TFminiRx_Buffer,VL53L1RX_Buffer[7];
...
void TFminiUartInit(void)
{
  	...
  	TFminiReceivexfer.dataSize =1;
		TFminiReceivexfer.data = &TFminiRx_Buffer;
  	uart_set_handle(USART_1, &TFminiLpuartHandle, TFminiUART_CallBack, NULL, 0, TFminiReceivexfer.data, 1 );
}
void TFminiUART_CallBack(LPUART_Type *base, lpuart_handle_t *handle, status_t status, void *userData)
{
	static uint8 RecNum;
		if(kStatus_LPUART_RxIdle == status)
    {
			TFminiRx_Data[RecNum] = TFminiRx_Buffer;
			RecNum++;
			if(RecNum==1 && TFminiRx_Data[0]!=0x59)
			{
				RecNum=0;
			}
			if(RecNum== 9)
    ...
    handle->rxDataSize = TFminiReceivexfer.dataSize;
    handle->rxData = TFminiReceivexfer.data;
}
