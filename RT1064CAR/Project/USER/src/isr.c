...
uint16_t SuperSonic_Distance = 0;
uint8_t SuperSonic_UpdateState = 0;
...
{
	//超声波测距用外部中断 使用D15口& PIT通道3
	if(GET_GPIO_FLAG(D15))
	{
		CLEAR_GPIO_FLAG(D15);
		if(gpio_get(D15) == 1)
		{
		...
		{
			SuperSonic_HighTime = pit_get(PIT_CH3);
			SuperSonic_Distance = SuperSonic_HighTime / 200;
			SuperSonic_UpdateState = 1;
			pit_close(PIT_CH3);
		}
	}
}
