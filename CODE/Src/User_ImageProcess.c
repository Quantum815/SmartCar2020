...
#include "..\CODE\Inc\User_ImageProcess.h"
...
void ImagePretreatment(void)
{
    for (int i=0; i<=MT9V03X_H-1; i++)
    {
        for(int j=0; j<=MT9V03X_W-1; j++)
        {
                ...
                {
                	mt9v03x_image[i][j] = mt9v03x_image[i][j] + 0;
                }
...
void FindMidLine(void)
{
    for (int i=0; i<=MT9V03X_H-1; i++)
    {
        if(i <= 10)
        {
            ...
            {
                if(mt9v03x_image[i][k] == 0)
                {
                    x2 = k;
                    break;
                }
            }
            mt9v03x_image[i][(x1 + x2) / 2] = 0;
        }
        else
        {
            for (int f=(x1+x2)/2; f>=0; f--)
            {
                if (mt9v03x_image[i][f] == 0)
                {
                    x3 = f;
                    break;
                }
            }
            for (int g=(x1+x2)/2; g<=188; g++)
            {
                if (mt9v03x_image[i][g] == 0)
                {
                    x4 = g;
                    break;
                }
            }
            mt9v03x_image[i][(x3 + x4) / 2] = 0;
			if(x4 == 188 && x3==0)
			{
				x2 = x4;
				x1 = x3;
			}
            if(myabs(((x3 + x4) / 2) - ((x1 + x2) / 2)) < 5)
            {
                x2 = x4;
                x1 = x3;
            }
	...
	/*if(DebugRx_Buffer == 0x06)
	{
		for(uint8 i=0; i<=MT9V03X_H-1; i++)
			for(uint8 j=0; j<=MT9V03X_W-1; j++)
				DisplayImage[i][j]=mt9v03x_csi_image[i][j];
		ips114_displayimage032_zoom(DisplayImage[0], MT9V03X_CSI_W, MT9V03X_H, 240, 135);
	}*/
}
