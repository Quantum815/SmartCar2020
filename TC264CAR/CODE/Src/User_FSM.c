...
//    		(ADCValueHandle(0) > ADCvalueLL || ADCValueHandle(4) > ADCvalueRR) && ReturnFSMState(&CarFSM) == GoLine && RoundaboutCount==0)//检测进圆 (ADCValueHandle(1) >= ADCvalueCL ||
//    else if(ADCValueHandle(2) >= ADCvalueC && ReturnFSMState(&CarFSM) == GoLine && RoundaboutCount==0)//检测进圆 (ADCValueHandle(1) >= ADCvalueCL ||
//    {
//    	IntoRoundaboutCount++;
...
////    {
//    else if(ADCValueHandle(2) >= ADCvalueC && ReturnFSMState(&CarFSM) == PassRoundabout && PassingRoundaboutFlag && GetDistance(1)>=10.5)//检测出圆
//    {
////    	PassingRoundaboutFlag=0;
